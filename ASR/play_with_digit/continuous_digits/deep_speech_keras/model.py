...
CONV_FILTERS = 32
BN_EPSILON = 1e-5  # batch_normalization config
BN_DECAY = 0.997
...
class MyModel(object):
    def __init__(self, flags_obj, num_classes):
        ...
        :param flags_obj: user_flags中的flags
        :param num_classes: true_lables + 1
        """
    ...
    def build_model(self):
        # spectrogram特征，(time_steps, frequencies), 129来自dataset.py中的get_spectrogram_and_seq_len函数
        features = Input(shape=(None, 129), dtype="float32", name="features")
        # 这里传入的是计算好的经过卷积的seq_len，尝试过传入原本的seq_len然后compute the seq_len_after_conv on fly,
        # 但是这个计算需要取features的shape，而None这里好像会引起错误。
        #  TODO 还未尝试将此计算包装成一个layer
        ...
        # readme （2）
        x = Lambda(self.expand_dim, name="expand_dim_before_cnn")(features)
